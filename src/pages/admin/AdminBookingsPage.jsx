  ...
  export default function AdminBookingsPage() {
    ...
    try {
      ...
      const firstRes = await api.get(`/admin/bookings?page=1`);
      const firstPage = firstRes.data?.bookings;

      let allBookings = [...(firstPage?.data || [])];
      ...
      if (lastPage > 1) {
        ...
        for (let i = 2; i <= lastPage; i++) {
          requests.push(api.get(`/admin/bookings?page=${i}`));
        }

        const responses = await Promise.all(requests);

        responses.forEach((res) => {
          allBookings.push(...(res.data?.bookings?.data || []));
        });
      }
