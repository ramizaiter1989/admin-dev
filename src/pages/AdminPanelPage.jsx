...
export default function AdminPanelPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (filters.users.role && filters.users.role !== "all")
        params.role = filters.users.role;
      if (filters.users.status && filters.users.status !== "all") {
        // Convert string to boolean as per API documentation
        params.status =
          filters.users.status === "active" || filters.users.status === "true";
      }
  ...
  const fetchCars = async () => {
    ...
    try {
      ...
      if (filters.cars.status && filters.cars.status !== "all")
        params.status = filters.cars.status;
      if (filters.cars.car_accepted && filters.cars.car_accepted !== "all")
        params.car_accepted = filters.cars.car_accepted;
      if (filters.cars.agent_id && filters.cars.agent_id !== "all")
        params.agent_id = filters.cars.agent_id;
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      if (filters.bookings.status && filters.bookings.status !== "all")
        params.append("status", filters.bookings.status);
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      if (filters.payments.type && filters.payments.type !== "all")
        params.type = filters.payments.type;
  ...
  const fetchAnnouncements = async () => {
    ...
    try {
      const params = new URLSearchParams();
      if (
        filters.announcements.status &&
        filters.announcements.status !== "all"
      )
        params.append("status", filters.announcements.status);
  ...
  const fetchAppeals = async () => {
    ...
    try {
      const params = new URLSearchParams();
      if (filters.appeals.status && filters.appeals.status !== "all")
        params.append("status", filters.appeals.status);
      if (filters.appeals.priority && filters.appeals.priority !== "all")
        params.append("priority", filters.appeals.priority);
  ...
  const fetchAds = async () => {
    ...
    try {
      const params = new URLSearchParams();
      if (filters.ads.status && filters.ads.status !== "all")
        params.append("status", filters.ads.status);
  ...
  const fetchFeaturedCars = async () => {
    ...
    try {
      const params = new URLSearchParams();
      if (filters.featuredCars.status && filters.featuredCars.status !== "all")
        params.append("status", filters.featuredCars.status);
  ...
  const fetchHolidays = async () => {
    ...
    try {
      const params = new URLSearchParams();
      if (filters.holidays.car_id && filters.holidays.car_id !== "all")
        params.append("car_id", filters.holidays.car_id);
      if (filters.holidays.active && filters.holidays.active !== "all")
        params.append("active", filters.holidays.active);
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      const params = new URLSearchParams();
      if (filters.suggestions.status && filters.suggestions.status !== "all")
        params.append("status", filters.suggestions.status);
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const params = new URLSearchParams();
      if (filters.notifications.type && filters.notifications.type !== "all")
        params.append("type", filters.notifications.type);
      if (
        filters.notifications.is_read &&
        filters.notifications.is_read !== "all"
      )
        params.append("is_read", filters.notifications.is_read);
  ...
  const fetchOtps = async () => {
    ...
    try {
      const params = {};
      if (filters.otps.phone_number)
        params.phone_number = filters.otps.phone_number;
      if (filters.otps.expired && filters.otps.expired !== "all")
        params.expired = filters.otps.expired;
  ...
  const fetchStatistics = async (period = "month") => {
    try {
      const { data } = await api.get(
        `/admin/reports/statistics?period=${period}`,
      );
  ...
  const fetchChartData = async (period = "month") => {
    try {
      const { data } = await api.get(
        `/admin/reports/chart-data?period=${period}`,
      );
      setChartData(data);
    } catch (error) {
  ...
  const createFeaturedCarHandler = async (data) => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to create featured car",
      );
    }
  };
  ...
  const fetchAgentAppFeesBalance = async (agentId) => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to fetch agent fee data",
      );
      return null;
    }
  };
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to update agent fee percentage",
      );
    }
  };
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              size="icon"
                              onClick={async () => {
                                const fullDetails = await fetchUserDetails(
                                  user.id,
                                );
                                if (fullDetails) {
                    ...
                    {cars.data?.map((car) => (
                      <TableRow key={car.id}>
                        ...
                        <TableCell>
                          {/* Start Tony Update  */}
                          <span
                            className="car-make-model-hover"
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              size="icon"
                              onClick={async () => {
                                const fullDetails = await fetchCarDetails(
                                  car.id,
                                );
                                if (fullDetails) {
                                  setSelectedItem(fullDetails);
                      ...
                      <TableRow key={booking.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline">
                            {booking.booking_request_status}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <Badge
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              size="icon"
                              onClick={async () => {
                                const fullDetails = await fetchBookingDetails(
                                  booking.id,
                                );
                                if (fullDetails) {
                                  setSelectedItem(fullDetails);
                  ...
                  onClick={() => {
                    const columns = [
                      { key: "id", label: "ID" },
                      {
                        key: "user",
                        label: "User",
                        transform: (p) => p.user?.name || "N/A",
                      },
                      {
                        key: "amount",
                        label: "Amount",
                        transform: (p) => `$${p.amount}`,
                      },
                      { key: "type", label: "Type" },
                  ...
                  <TableBody>
                    {featuredCars.data?.map((featured) => (
                      <TableRow key={featured.id}>
                        ...
                        <TableCell>
                          {new Date(featured.start_at).toLocaleDateString()}
                        </TableCell>
                        <TableCell>
                          {new Date(featured.expire_at).toLocaleDateString()}
                        </TableCell>
                        <TableCell>
                          <Button
                            variant="ghost"
                      ...
                      otps.data?.map((otp) => {
                        ...
                        return (
                          <TableRow key={otp.id}>
                            ...
                            <TableCell>
                              {new Date(otp.expired_at).toLocaleString()}
                            </TableCell>
                            <TableCell>
                              {isExpired ? (
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedItem.first_name || "N/A"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">Last Name</Label>
                      <p className="text-sm">
                        {selectedItem.last_name || "N/A"}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Badge
                        variant={selectedItem.status ? "default" : "secondary"}
                      >
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          selectedItem.is_locked ? "destructive" : "default"
                        }
                      >
                    ...
                    <div className="border-t pt-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-sm">
                            {selectedItem.client.license_number || "N/A"}
                          </p>
                        </div>
                        <div>
                          <Label className="text-xs">Profession</Label>
                          <p className="text-sm">
                            {selectedItem.client.profession || "N/A"}
                          </p>
                        </div>
                        <div>
                          <Label className="text-xs">Average Salary</Label>
                          <p className="text-sm">
                            {selectedItem.client.avg_salary || "N/A"}
                          </p>
                        </div>
                        <div>
                          <Label className="text-xs">Promo Code</Label>
                          <p className="text-sm">
                            {selectedItem.client.promo_code || "N/A"}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="border-t pt-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-sm">
                            {selectedItem.agent.business_type || "N/A"}
                          </p>
                        </div>
                        <div>
                          <Label className="text-xs">Company Number</Label>
                          <p className="text-sm">
                            {selectedItem.agent.company_number || "N/A"}
                          </p>
                        </div>
                        <div>
                          <Label className="text-xs">Profession</Label>
                          <p className="text-sm">
                            {selectedItem.agent.profession || "N/A"}
                          </p>
                        </div>
                        <div>
                          <Label className="text-xs">Website</Label>
                          <p className="text-sm">
                            {selectedItem.agent.website || "N/A"}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              {modalType === "edit-user" && selectedItem && (
                <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        value={
                          selectedItem.birth_date
                            ? new Date(selectedItem.birth_date)
                                .toISOString()
                                .split("T")[0]
                            : ""
                        }
                    ...
                    <div className="border-t pt-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label className="text-xs">License Number</Label>
                          <Input
                            value={selectedItem.client.license_number || "N/A"}
                            disabled
                          />
                        </div>
                        <div>
                          <Label className="text-xs">Profession</Label>
                          <Input
                            value={selectedItem.client.profession || "N/A"}
                            disabled
                          />
                        </div>
                        <div>
                          <Label className="text-xs">Average Salary</Label>
                          <Input
                            value={selectedItem.client.avg_salary || "N/A"}
                            disabled
                          />
                        </div>
                        <div>
                          <Label className="text-xs">Promo Code</Label>
                          <Input
                            value={selectedItem.client.promo_code || "N/A"}
                            disabled
                          />
                        </div>
                      </div>
                  ...
                  {selectedItem.agent && (
                    <div className="border-t pt-4">
                      <Label className="text-sm font-semibold mb-2 block">
                        Agent/Agency Details (Read-only)
                      </Label>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label className="text-xs">Business Type</Label>
                          <Input
                            value={selectedItem.agent.business_type || "N/A"}
                            disabled
                          />
                        </div>
                        <div>
                          <Label className="text-xs">Company Number</Label>
                          <Input
                            value={selectedItem.agent.company_number || "N/A"}
                            disabled
                          />
                        </div>
                        <div>
                          <Label className="text-xs">Profession</Label>
                          <Input
                            value={selectedItem.agent.profession || "N/A"}
                            disabled
                          />
                        </div>
                        <div>
                          <Label className="text-xs">Website</Label>
                          <Input
                            value={selectedItem.agent.website || "N/A"}
                            disabled
                          />
                        </div>
                      </div>
                  ...
                  <div className="border-t pt-4 space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          setSelectedItem({ ...selectedItem, status: checked })
                        }
                      />
                    </div>
                    <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        checked={
                          selectedItem.update_access !== undefined
                            ? selectedItem.update_access
                            : true
                        }
                        onCheckedChange={(checked) =>
                          setSelectedItem({
                            ...selectedItem,
                            update_access: checked,
                          })
                        }
                      />
                    </div>
                  ...
                  {(selectedItem.role === "agency" ||
                    selectedItem.role === "agent") && (
                    <div className="border-t pt-4 space-y-3">
                      ...
                      {appFeesData[selectedItem.id] ? (
                        <div className="space-y-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-semibold">
                              ${appFeesData[selectedItem.id].total_balance || 0}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            ...
                            <span className="text-green-600">
                              $
                              {appFeesData[selectedItem.id].breakdown
                                ?.paid_amount || 0}
                            </span>
                          </div>
                          <div className="flex justify-between">
                        ...
                        <p className="text-sm text-gray-500">
                          Click "Refresh Balance" to load app fees data
                        </p>
                      )}
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            onClick={() => {
                              if (
                                selectedItem.app_fees_percentage !== undefined
                              ) {
                                updateAgentAppFeesPercentage(
                                  selectedItem.id,
                                  selectedItem.app_fees_percentage,
                                );
                              } else {
                                toast.error(
                                  "Please enter a valid app fees percentage",
                                );
                              }
                            }}
                            variant="outline"
                        ...
                        <p className="text-xs text-gray-500">
                          Enter percentage (0-100), e.g., 10.00 for 10%
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedItem.license_plate || "N/A"}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedItem.mileage?.toLocaleString() || "N/A"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">Fuel Type</Label>
                      <p className="text-sm">
                        {selectedItem.fuel_type || "N/A"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedItem.wheels_drive || "N/A"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">Category</Label>
                      <p className="text-sm">
                        {selectedItem.car_category || "N/A"}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          selectedItem.car_accepted ? "default" : "secondary"
                        }
                      >
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          selectedItem.with_driver ? "default" : "secondary"
                        }
                      >
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          selectedItem.is_delivered ? "default" : "secondary"
                        }
                      >
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedItem.min_rental_days || "N/A"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">
                  ...
                  {selectedItem.car_add_on &&
                    selectedItem.car_add_on.length > 0 && (
                      <div>
                  ...
                  {selectedItem.agent && (
                    <div className="border-t pt-4">
                      ...
                      <p className="text-sm">
                        {selectedItem.agent.username ||
                          selectedItem.agent.first_name}{" "}
                        {selectedItem.agent.last_name}
                      </p>
                      <p className="text-xs text-gray-500">
                        {selectedItem.agent.phone_number}
                      </p>
                    </div>
                  )}
                  <div className="flex gap-2 pt-4">
                ...
                <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <Label>Mileage</Label>
                      <Input
                        value={selectedItem.mileage?.toLocaleString() || ""}
                        disabled
                      />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Input
                        value={`$${selectedItem.daily_rate || "0.00"}`}
                        disabled
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={`$${selectedItem.holiday_rate || "N/A"}`}
                        disabled
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={selectedItem.min_rental_days || ""}
                        disabled
                      />
                    </div>
                    <div>
                ...
                <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedItem.car?.make} {selectedItem.car?.model} (
                        {selectedItem.car?.year})
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">Agent</Label>
                      <p className="text-sm">
                        {selectedItem.car?.agent?.username ||
                          selectedItem.car?.agent?.first_name ||
                          "N/A"}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold">
                        ${selectedItem.total_booking_price || "0.00"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">
                      ...
                      <Badge variant="outline">
                        {selectedItem.booking_request_status || "N/A"}
                      </Badge>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          selectedItem.is_paid_online ? "default" : "secondary"
                        }
                      >
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedItem.pickup_location || "N/A"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedItem.reason_of_booking || "N/A"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">
                      ...
                      <p className="text-sm">
                        ${selectedItem.extra_charge || "0.00"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {selectedItem.cancelation_reason || "N/A"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-semibold">
                ...
                <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        value={selectedItem.payment_status || "N/A"}
                        disabled
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={selectedItem.payment_method || "N/A"}
                        disabled
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={selectedItem.is_paid_online ? "Yes" : "No"}
                        disabled
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={selectedItem.is_delivered ? "Yes" : "No"}
                        disabled
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={selectedItem.pickup_location || "N/A"}
                        disabled
                      />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Input
                        value={selectedItem.reason_of_booking || "N/A"}
                        disabled
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={`$${selectedItem.extra_charge || "0.00"}`}
                        disabled
                      />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Input
                        value={selectedItem.cancelation_reason || "N/A"}
                        disabled
                      />
                    </div>
                    <div>
                  ...
                  <div className="flex gap-2 pt-4">
                    <Button
                      onClick={() =>
                        updateBooking(selectedItem.id, {
                          booking_request_status:
                            selectedItem.booking_request_status,
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setSelectedItem({ ...selectedItem, priority: value })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setSelectedItem({ ...selectedItem, online: checked })
                      }
                    />
                  </div>
                  <Button
                    onClick={() =>
                      updateAd(selectedItem.id, {
              ...
              {modalType === "edit-suggestion" && selectedItem && (
                <div className="space-y-4">
                  ...
                  <Button
                    onClick={() =>
                      updateSuggestion(selectedItem.id, {
                        status: selectedItem.status,
                      })
                    }
                  >
              ...
              {(modalType === "create-holiday" ||
                modalType === "edit-holiday") && (
                <HolidayForm
              ...
              {/* Create Ad Modal */}
              {modalType === "create-ad" && (
                <AdForm onSubmit={createAdHandler} loading={loading} />
              )}
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) =>
            setFormData({ ...formData, percentage_increase: e.target.value })
          }
          placeholder="20.00"
          required
        />
  ...
  const handleSubmit = () => {
    ...
    const submitData = {
      ...
      scheduled_at: formData.scheduled_at
        ? formatDateForAPI(formData.scheduled_at)
        : undefined,
      expires_at: formData.expires_at
        ? formatDateForAPI(formData.expires_at)
        : undefined,
          ...
          {["client", "agency", "agent", "admin"].map((role) => (
            <div key={role} className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) =>
                  handleTargetAudienceChange(role, e.target.checked)
                }
                className="rounded"
              />
              <Label
...
function NotificationForm({ onSubmit }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Button onClick={() => onSubmit(formData)}>Send Notification</Button>
    </div>
  );
}
// Start Tony Update
...
function carTooltip(car) {
  return [
    ...
    `wheels_drive: ${car.wheels_drive ?? "N/A"}`,
    `Daily_rate: ${car.daily_rate ?? "N/A"}`,
